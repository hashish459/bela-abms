...
type Row = { id: string; number: string; date: string; supplier: string; taxable: string; vat: string; grandTotal: string; status: string };
...
type InvItem = {
  ...
  discount: number; exciseDuty: number; customDuty: number; taxRateId: string; isNonTaxable: boolean;
};
type Invoice = { id: string; number: string; supplier: string; grandTotal: string; items: InvItem[] };

export function DebitNoteWorkspace({
  initial, invoices, canCreate,
}: { initial: List; invoices: Invoice[]; canCreate: boolean }) {
  ...
  return (
    <>
      <PageHeader
        crumbs={["Purchase", "Debit Notes"]}
        title="Debit Notes"
        action={canCreate && invoices.length > 0 && (
      ...
      <p className="mb-3 text-xs text-muted">
        A debit note reverses a purchase invoice (in full or in part): it returns stock to the
        supplier, reverses the input VAT and inventory value, and reduces what you owe.
      </p>

      {initial.rows.length === 0 ? (
        <EmptyState title="No debit notes" hint="Issue one against a purchase invoice to record a return." />
      ) : (
...
function DebitNoteForm({
  invoices, onClose, onSaved,
}: { invoices: Invoice[]; onClose: () => void; onSaved: () => void }) {
  ...
  async function save() {
    const payload = lines
      .filter((l) => l.qty > 0)
      .map((l) => ({
        productId: l.productId || undefined, description: l.description, hsCode: l.hsCode || undefined,
        qty: l.qty, rate: l.rate, discount: l.discount || 0,
        exciseDuty: l.exciseDuty || 0, customDuty: l.customDuty || 0,
        taxRateId: l.taxRateId || undefined, isNonTaxable: l.isNonTaxable || !l.taxRateId,
      }));
    if (!reversesDocId) return toast("Select the invoice to debit", "err");
    if (payload.length === 0) return toast("Set the return quantity on at least one line", "err");

    setSaving(true);
    const res = await api<{ debitNote: { number: string } }>("/api/purchase/debit-notes", {
      method: "POST",
      body: JSON.stringify({ date, reversesDocId, notes: notes || undefined, lines: payload }),
    });
    setSaving(false);
    if (!res.ok) return toast(res.error.message, "err");
    toast(`Debit note ${res.data.debitNote.number} issued`);
    onSaved();
  }

  return (
    <Modal open onClose={onClose} title="New Debit Note" wide>
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          <Field label="Against invoice" required>
            <select value={reversesDocId} onChange={(e) => pickInvoice(e.target.value)} className={inputClass}>
              ...
              {invoices.map((i) => (
                <option key={i.id} value={i.id}>{i.number} — {i.supplier} ({i.grandTotal})</option>
              ))}
            </select>
          </Field>
                  ...
                  return (
                    <tr key={l.key}>
                      ...
                      <td className="px-2 py-1.5">
                        <input
                          type="number" step="0.001" min="0" max={invoicedQty} value={l.qty}
                          onChange={(e) =>
                            setLines((ls) => ls.map((x) => (x.key === l.key ? { ...x, qty: Math.min(Number(e.target.value) || 0, invoicedQty) } : x)))
                          }
      ...
      <div className="mt-4 flex justify-end gap-2 border-t border-border pt-3">
        ...
        <Button loading={saving} onClick={save} disabled={!reversesDocId}>Issue Debit Note</Button>
      </div>
    </Modal>
  );
}
