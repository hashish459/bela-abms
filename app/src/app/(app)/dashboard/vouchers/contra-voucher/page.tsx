...
export const metadata = { title: "Contra Voucher — Bela ABMS" };

export default async function ContraVoucherPage() {
  const s = (await getSession())!;
  if (!can(s.permissions, "vouchers.contra_voucher", "read")) redirect("/dashboard");

  const fyId = await activeFiscalYearId(s.companyId!).catch(() => null);
  const list = await listVouchers(s.companyId!, fyId, "CONTRA", { page: 1 });
  ...
  return (
    <VoucherWorkspace
      ...
      initial={list}
      canCreate={can(s.permissions, "vouchers.contra_voucher", "create")}
    />
  );
}
