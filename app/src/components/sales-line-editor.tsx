...
export type TaxOpt = { id: string; name: string; ratePct: number; isNoTax: boolean };
...
export function SalesLineEditor({
  ...
  lines: EditorLine[];
  setLines: (fn: (ls: EditorLine[]) => EditorLine[]) => void;
  taxRates: TaxOpt[];
  invoiceDiscount: string;
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      ...
      try {
        const res = await fetch("/api/sales/calc", {
          method: "POST",
    ...
    <div className="space-y-3">
        ...
        <table className="w-full text-sm">
          <thead className="bg-background text-left text-xs text-muted">
            <tr>
              ...
              <th className="w-28 px-2 py-2 font-medium">Tax</th>
              ...
              <tr key={l.key}>
                ...
                <td className="px-2 py-1.5">
                  <input type="number" step="0.001" value={l.qty}
                    onChange={(e) => patch(l.key, { qty: e.target.value })} className={`${inputClass} text-right`} />
                </td>
                <td className="px-2 py-1.5">
                  <input type="number" step="0.01" value={l.rate}
                    onChange={(e) => patch(l.key, { rate: e.target.value })} className={`${inputClass} text-right`} />
                </td>
                <td className="px-2 py-1.5">
                  <input type="number" step="0.01" value={l.discount}
                    onChange={(e) => patch(l.key, { discount: e.target.value })} className={`${inputClass} text-right`} />
                </td>
                <td className="px-2 py-1.5">
      ...
      <div className="ml-auto max-w-xs space-y-1 text-sm">
        ...
        <div className="flex items-center justify-between gap-2">
          ...
          <input type="number" step="0.01" value={invoiceDiscount}
